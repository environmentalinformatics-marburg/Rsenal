...
#' 
#' @examples
#' pred_vals <- rnorm(1000, 200, 10)
#' obs_vals <- rnorm(1000, 200, 10)
#' 
#' result <- predictionStats(sort(pred_vals), sort(obs_vals), plot = TRUE)
#' result$stats
#' result$plot
#' 
...
predictionStats <- function(prd, obs, adj.rsq = TRUE, plot = FALSE) {
  ...
  if (plot) {
    ...
    err_plt <- Hmisc::Dotplot(nms ~ Cbind(fit, 
                          ...
                          par.settings = envinmr.theme(),
                          cex = 1.2, as.table = TRUE) + 
      layer_(panel.lines(x = c(0, 0), 
                        y = c(0, 3.25), 
                        lty = 3, lwd = 1)) +
      layer_(panel.lines(x = c(1, 1), 
                         y = c(3.75, 4.25), 
                         lty = 3, lwd = 1))
